...
def agg_loss(loss_mat: torch.Tensor, loss_mask: torch.Tensor, loss_agg_mode: str):
    """
    [Auto-Fix] Robust agg_loss with dimension alignment.
    """
    # --- 维度自动对齐 (针对 511 vs 512 问题) ---
    if loss_mat.shape[-1] != loss_mask.shape[-1]:
        min_len = min(loss_mat.shape[-1], loss_mask.shape[-1])
        # 截断到相同长度
        loss_mat = loss_mat[..., :min_len]
    ...
    if loss_agg_mode == "token-mean":
        ...
        # 避免除以零
        sum_mask = torch.sum(loss_mask, dim=-1)
        sum_mask = torch.where(sum_mask == 0, torch.ones_like(sum_mask), sum_mask)
        seq_losses = torch.sum(loss_mat * loss_mask, dim=-1) / sum_mask
        loss = torch.mean(seq_losses)  # seq-mean
    elif loss_agg_mode == "seq-mean-token-sum-norm":
        seq_losses = torch.sum(loss_mat * loss_mask, dim=-1)
        loss = torch.sum(seq_losses) / max(loss_mask.shape[-1], 1) 
    else:
...
def compute_policy_loss_vanilla(
    ...
    """
    [Auto-Fix] Hardcoded robust version.
    Aligns dimensions by truncating the longer tensor (ignoring extra reward tokens).
    """
    assert config is not None
    
    # --- 核心修复逻辑 ---
    min_len = min(log_prob.shape[-1], response_mask.shape[-1])
    
    # 强制对齐所有 Tensor 到最短长度
    # 这会丢弃 response_mask/advantages 中多余的末尾 Token
    # 这是安全的，因为 log_prob 既然没有那一位，那一位本身就无法产生梯度
    if log_prob.shape[-1] != min_len: log_prob = log_prob[:, :min_len]
    if old_log_prob.shape[-1] != min_len: old_log_prob = old_log_prob[:, :min_len]
    if advantages.shape[-1] != min_len: advantages = advantages[:, :min_len]
    if response_mask.shape[-1] != min_len: response_mask = response_mask[:, :min_len]
