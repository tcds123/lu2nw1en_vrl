...
def validate_config(
    ...
    if not config.actor_rollout_ref.actor.use_dynamic_bsz:
        if use_reference_policy:
            ...
            # --- 关键修复：使用 OmegaConf.select 安全地访问键 ---
            check_mutually_exclusive(
                OmegaConf.select(config, "actor_rollout_ref.ref.log_prob_micro_batch_size", default=None),
                OmegaConf.select(config, "actor_rollout_ref.ref.log_prob_micro_batch_size_per_gpu", default=None),
        ...
        # --- 关键修复：使用 OmegaConf.select 安全地访问键 ---
        check_mutually_exclusive(
            OmegaConf.select(config, "actor_rollout_ref.rollout.log_prob_micro_batch_size", default=None),
            OmegaConf.select(config, "actor_rollout_ref.rollout.log_prob_micro_batch_size_per_gpu", default=None),
    ...
    if config.reward_model.enable and not config.reward_model.use_dynamic_bsz:
        # --- 关键修复：使用 OmegaConf.select 安全地访问键 ---
        check_mutually_exclusive(
            OmegaConf.select(config, "reward_model.micro_batch_size", default=None),
            OmegaConf.select(config, "reward_model.micro_batch_size_per_gpu", default=None),
            "reward_model"
        )
    ...
    # check eval config
    # 修复：安全检查 val_kwargs 是否存在
    val_kwargs = OmegaConf.select(config, "actor_rollout_ref.rollout.val_kwargs", default=None)
    if val_kwargs and val_kwargs.do_sample:
