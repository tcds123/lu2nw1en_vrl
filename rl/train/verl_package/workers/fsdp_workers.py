...
def get_free_port() -> int:
    """获取一个未被占用的端口（确保分布式通信可用）"""
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        s.bind(("", 0))  # 绑定到所有可用网络接口，端口设为0（系统自动分配空闲端口）
        s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)  # 允许端口重用，避免TIME_WAIT冲突
        return s.getsockname()[1]  # 返回系统分配的空闲端口
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        if not torch.distributed.is_initialized():
           ...
           # --- [关键修改] 兼容公司托管环境的端口协调机制 ---
            
            # 1. 确定 Master Address
            # 优先信任 Ray/K8s 注入的 MASTER_ADDR (通常是 Pod IP 或 Head Node IP)
            # 如果没有，且是单机训练 (nnodes=1)，回退到 127.0.0.1
            master_addr = os.environ.get("MASTER_ADDR", "127.0.0.1")
            
            # 2. 动态端口协调 (解决 EADDRINUSE)
            # 仅当单机多卡时使用文件锁 (nnodes=1)，多机环境应信任 MASTER_PORT 或使用 KVStore
            # 由于您目前配置 nnodes=1，此逻辑最稳健
            wg_prefix = os.environ.get("WG_PREFIX", "default")
            port_file_path = Path(f"/tmp/verl_dist_port_{wg_prefix}.txt")
            
            if rank == 0:
                # Rank 0 负责寻找真正空闲的端口
                dynamic_port = get_free_port()
                print(f"🚀 [Rank 0] Selected Free Port: {dynamic_port} (Binding to {master_addr})")
                
                try:
                    with open(port_file_path, "w") as f:
                        f.write(str(dynamic_port))
                except Exception as e:
                    raise RuntimeError(f"Rank 0 failed to write port file: {e}")
            else:
                # 其他 Rank 等待文件生成
                print(f"⏳ [Rank {rank}] Waiting for port file: {port_file_path}")
                wait_start = time.time()
                while not port_file_path.exists():
                    time.sleep(0.1)
                    if time.time() - wait_start > 60:
                        raise TimeoutError(f"Rank {rank} timed out waiting for port file.")
                ...
                if dynamic_port is None:
                    raise RuntimeError(f"Rank {rank} failed to read valid port from {port_file_path}")
                
                print(f"✅ [Rank {rank}] Found port: {dynamic_port}")

            # 3. 初始化进程组
            init_method = f"tcp://{master_addr}:{dynamic_port}"
            print(f"🔗 [Rank {rank}] Connecting to: {init_method}")
            ...
            # # 1. 确定master地址（优先从环境变量获取，默认localhost）
            # master_addr = os.environ.get("MASTER_ADDR", "localhost")
            # # 2. 动态生成空闲端口（仅rank=0需要生成，其他进程复用该端口）
            # # 注：多进程场景下，确保所有进程使用同一个端口，这里通过rank=0生成后传递
            # if rank == 0:
            #     dynamic_port = get_free_port()
            #     # 将端口存入环境变量，供其他进程读取（避免多进程端口不一致）
            #     os.environ["DYNAMIC_MASTER_PORT"] = str(dynamic_port)
            # else:
            #     # 非0进程等待端口被设置（避免竞争）
            #     while "DYNAMIC_MASTER_PORT" not in os.environ:
            #         import time
            #         time.sleep(0.1)
            #     dynamic_port = int(os.environ["DYNAMIC_MASTER_PORT"])
            
            # # 3. 构造包含动态端口的init_method
            # init_method = f"tcp://{master_addr}:{dynamic_port}"
            ...
            #     world_size=world_size,
            #     timeout=datetime.timedelta(seconds=self.config.get("nccl_timeout", 600)),
            #     init_method=init_method,  # 替换为动态生成的init_method
            # )
